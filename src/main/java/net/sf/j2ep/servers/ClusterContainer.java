...
public abstract class ClusterContainer extends ServerContainerBase implements ServerStatusListener {
    ...
    public ClusterContainer() {
        ...
        statusChecker.start();
        log = LoggerFactory.getLogger(ClusterContainer.class);
    }
    ...
    public Server getServer(HttpServletRequest request) {
        ...
        if (server == null || !server.online()) {
            ...
            if (log.isDebugEnabled()) log.debug("Server found in session");
        }

        if (server.online()) {
            if (log.isDebugEnabled()) log.debug("Using id " + server.getServerId() + " for this request");
        } else {
